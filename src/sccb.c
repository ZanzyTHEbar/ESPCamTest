...
{
    log_i("pin_sda %d pin_scl %d", pin_sda, pin_scl);
    i2c_config_t conf;
...
uint8_t SCCB_Probe(void)
{
    ...
    //     if( ret == ESP_OK) {
    //         log_w( "Found I2C Device at 0x%02X", i);
    //     }
    // }
...
uint8_t SCCB_Read(uint8_t slv_addr, uint8_t reg)
{
    ...
    if (ret != ESP_OK)
    {
        log_e("SCCB_Read Failed addr:0x%02x, reg:0x%02x, data:0x%02x, ret:%d", slv_addr, reg, data, ret);
    }
    return data;
}
...
uint8_t SCCB_Write(uint8_t slv_addr, uint8_t reg, uint8_t data)
{
    ...
    if (ret != ESP_OK)
    {
        log_e("SCCB_Write Failed addr:0x%02x, reg:0x%02x, data:0x%02x, ret:%d", slv_addr, reg, data, ret);
    }
