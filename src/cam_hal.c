                    ...
                    if (cnt || !cam_obj->jpeg_mode || cam_obj->psram_mode) {
                        ...
                        if (cam_obj->psram_mode) {
                            ...
                            if (frame_buffer_event->len != cam_obj->fb_size) {
                                ...
                                log_e( "FB-SIZE: %u != %u", frame_buffer_event->len, cam_obj->fb_size);
                            }
                        }
                        //send frame
...
static esp_err_t cam_dma_config()
{
    ...
    log_i( "buffer_size: %d, half_buffer_size: %d, node_buffer_size: %d, node_cnt: %d, total_cnt: %d", 
    ...
    for (int x = 0; x < cam_obj->frame_cnt; x++) {
        ...
        if (cam_obj->psram_mode) {
            ...
            log_i( "Frame[%d]: Offset: %u, Addr: 0x%08X", x, cam_obj->frames[x].fb_offset, (uint32_t)cam_obj->frames[x].fb.buf);
...
camera_fb_t *cam_take(TickType_t timeout)
{
    ...
    if (dma_buffer) {
        ...
        log_i( "Failed to get the frame on time!");
    }
    return NULL;
}
