...
static int write_reg(uint8_t slv_addr, const uint16_t reg, uint8_t value)
{
    ...
    {
        log_i("NEW REG 0x%04x: 0x%02x to 0x%02x", reg, (uint8_t)old_value, value);
        ret = SCCB_Write16(slv_addr, reg, value);
    }
    else
    {
...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pll_pre_div, bool pll_root_2x, int pll_seld5, bool pclk_manual, int pclk_div)
{
    ...
    log_i("Calculated VCO: %d Hz, PLLCLK: %d Hz, SYSCLK: %d Hz, PCLK: %d Hz", VCO * 1000, PLLCLK, SYSCLK, PCLK);
    return SYSCLK;
}
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    {
        log_w("Invalid framesize: %u", framesize);
    ...
    if (sensor->status.binning)
    {
        ret = write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, settings.total_x, (settings.total_y / 2) + 1) || write_addr_reg(sensor->slv_addr, X_OFFSET_H, 8, 2);
    }
    else
    {
        ret = write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, settings.total_x, settings.total_y) || write_addr_reg(sensor->slv_addr, X_OFFSET_H, 16, 6);
    }
    ...
    if (ret == 0)
    {
        log_d("Set framesize to: %ux%u", w, h);
    }
    return ret;

fail:
    sensor->status.framesize = old_framesize;
    log_e("Setting framesize to: %ux%u failed", w, h);
    return ret;
}
...
static int set_aec_value(sensor_t *sensor, int value)
{
    ...
    if (max_val < 0)
    {
        log_e("Could not read max aec_value");
        return -1;
    }
    if (value > max_val)
    {
    ...
    if (ret == 0)
    {
        log_d("Set aec_value to: %d / %d", value, max_val);
