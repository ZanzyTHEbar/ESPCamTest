...
static esp_err_t camera_probe(const camera_config_t *config, camera_model_t *out_camera_model)
{
    *out_camera_model = CAMERA_NONE;
    ...
    {
        log_d("Enabling XCLK output");
        CAMERA_ENABLE_OUT_CLOCK(config);
    }

    if (config->pin_sscb_sda != -1)
    {
        log_d("Initializing SSCB");
        SCCB_Init(config->pin_sscb_sda, config->pin_sscb_scl);
    }

    if (config->pin_pwdn >= 0)
    {
        log_d("Resetting camera by power down line");
    ...
    log_d("Searching for camera address");
    vTaskDelay(10 / portTICK_PERIOD_MS);
    ...
    log_i("Detected camera at address=0x%02x", slv_addr);
    ...
    else if (slv_addr == NT99141_SCCB_ADDR)
    {
        ...
        {
            log_e("NT99141: only XCLK under 10MHz is supported, and XCLK is now set to 10M");
            s_state->sensor.xclk_freq_hz = 10000000;
        }
    }
    vTaskDelay(10 / portTICK_PERIOD_MS);
    log_i("Camera PID=0x%02x VER=0x%02x MIDL=0x%02x MIDH=0x%02x",
    ...
    default:
        id->PID = 0;
        CAMERA_DISABLE_OUT_CLOCK();
        log_e("Detected camera not supported.");
        return ESP_ERR_NOT_SUPPORTED;
    }

    log_d("Doing SW reset of sensor");
...
esp_err_t esp_camera_init(const camera_config_t *config)
{
    ...
    if (err != ESP_OK)
    {
        log_e("Camera probe failed with error 0x%x(%s)", err, esp_err_to_name(err));
        goto fail;
    }
    ...
    {
        log_i("Detected OV3660 camera");
    }
    else if (camera_model == CAMERA_OV5640)
    {
        log_i("Detected OV5640 camera");
    }
    else if (camera_model == CAMERA_OV7670)
    {
        log_i("Detected OV7670 camera");
    }
    else if (camera_model == CAMERA_NT99141)
    {
        log_i("Detected NT99141 camera");
    }
    else
    {
        log_i("Camera not supported");
    ...
    if (err != ESP_OK)
    {
        log_e("Camera config failed with error 0x%x", err);
        goto fail;
    }

    s_state->sensor.status.framesize = frame_size;
    s_state->sensor.pixformat = pix_format;
    // log_d( "Setting frame size to %dx%d", s_state->width, s_state->height);
    if (s_state->sensor.set_framesize(&s_state->sensor, frame_size) != 0)
    {
        log_e("Failed to set frame size");
...
esp_err_t esp_camera_load_from_nvs(const char *key)
{
    ...
    {
        log_w("Error (%d) opening nvs key \"%s\"", ret, key);
        return ret;
    }
}
