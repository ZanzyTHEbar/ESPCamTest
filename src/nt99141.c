...
/* #if defined(ARDUINO_ARCH_ESP32) && defined(CONFIG_ARDUHAL_log_)
...
static int read_reg(uint8_t slv_addr, const uint16_t reg)
{
    ...
    if (ret < 0)
    {
        log_e("READ REG 0x%04x FAILED: %d", reg, ret);
    }
...
static int write_reg(uint8_t slv_addr, const uint16_t reg, uint8_t value)
{
    ...
    {
        log_d("NEW REG 0x%04x: 0x%02x to 0x%02x", reg, (uint8_t)old_value, value);
        ret = SCCB_Write16(slv_addr, reg, value);
    }
    else
    {
        log_d("OLD REG 0x%04x: 0x%02x", reg, (uint8_t)old_value);
        ret = SCCB_Write16(slv_addr, reg, value); // maybe not?
    }

    if (ret < 0)
    {
        log_e("WRITE REG 0x%04x FAILED: %d", reg, ret);
    }
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    default:
        log_e("Unsupported pixformat: %u", pixformat);
        return -1;
    }
    ...
    if (ret == 0)
    {
        ...
        log_d("Set pixformat to: %u", pixformat);
    }
...
static int set_image_options(sensor_t *sensor)
{
    ...
    {
        log_e("Setting Image Options Failed");
        ret = -1;
    }

    log_d("Set Image Options: Compression: %u, Binning: %u, V-Flip: %u, H-Mirror: %u, Reg-4514: 0x%02x",
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    {
        log_d("Set FRAMESIZE_HD");
        ret = write_regs(sensor->slv_addr, sensor_framesize_HD);
    }
    else
    {
        log_d("Dont suppost this size, Set FRAMESIZE_VGA");
        ret = write_regs(sensor->slv_addr, sensor_framesize_VGA);
    }
...
static int set_hmirror(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0)
    {
        log_d("Set h-mirror to: %d", enable);
    }
...
static int set_colorbar(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0)
    {
        ...
        log_d("Set colorbar to: %d", enable);
    }
...
static int set_gain_ctrl(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0)
    {
        log_d("Set gain_ctrl to: %d", enable);
...
static int set_exposure_ctrl(sensor_t *sensor, int enable)
{
    ...
    log_d("set_exposure_ctrl:enable");
    if (enable)
    {
        log_d("set_exposure_ctrl:enable");
    ...
    {
        log_d("set_exposure_ctrl:disable");
        ret = write_reg(sensor->slv_addr, 0x3201, (~(1 << 5)) & data);
    }

    if (ret == 0)
    {
        log_d("Set exposure_ctrl to: %d", enable);
...
{
    log_d("get_agc_gain can not be configured at present");
    return 0;
}
...
{
    log_d("set_agc_gain can not be configured at present");
    // log_d( "GAIN = %d\n", gain);
    int cnt = gain / 2;
...
{
    log_d("get_aec_value can not be configured at present");
    return 0;
}

static int set_aec_value(sensor_t *sensor, int value)
{
    log_d("set_aec_value can not be configured at present");
    int ret = 0;
    // log_d( " set_aec_value to: %d", value);
...
static int set_ae_level(sensor_t *sensor, int level)
{
    log_d("set_ae_level can not be configured at present");
    int ret = 0;
    ...
    if (ret)
    {
        log_e(" fail to set ae level: %d", ret);
    }
...
static int set_wb_mode(sensor_t *sensor, int mode)
{
    ...
    {
    case 1: // Sunny
        ret = write_reg16(sensor->slv_addr, 0x3290, 0x01) || write_reg16(sensor->slv_addr, 0x3291, 0x38) || write_reg16(sensor->slv_addr, 0x3296, 0x01) || write_reg16(sensor->slv_addr, 0x3297, 0x68) || write_reg16(sensor->slv_addr, 0x3060, 0x01);

        break;

    case 2: // Cloudy

        ret = write_reg16(sensor->slv_addr, 0x3290, 0x01) || write_reg16(sensor->slv_addr, 0x3291, 0x51) || write_reg16(sensor->slv_addr, 0x3296, 0x01) || write_reg16(sensor->slv_addr, 0x3297, 0x00) || write_reg16(sensor->slv_addr, 0x3060, 0x01);
        break;

    case 3: // INCANDESCENCE]
        ret = write_reg16(sensor->slv_addr, 0x3290, 0x01) || write_reg16(sensor->slv_addr, 0x3291, 0x30) || write_reg16(sensor->slv_addr, 0x3296, 0x01) || write_reg16(sensor->slv_addr, 0x3297, 0xCB) || write_reg16(sensor->slv_addr, 0x3060, 0x01);
        break;

    case 4: // FLUORESCENT
        ret = write_reg16(sensor->slv_addr, 0x3290, 0x01) || write_reg16(sensor->slv_addr, 0x3291, 0x70) || write_reg16(sensor->slv_addr, 0x3296, 0x01) || write_reg16(sensor->slv_addr, 0x3297, 0xFF) || write_reg16(sensor->slv_addr, 0x3060, 0x01);
        break;
...
static int set_awb_gain_dsp(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0)
    {
        ...
        log_d("Set awb_gain to: %d", enable);
...
static int set_special_effect(sensor_t *sensor, int effect)
{
    ...
    ret = write_reg(sensor->slv_addr, 0x32F1, regs[0]) || write_reg(sensor->slv_addr, 0x32F4, regs[1]) || write_reg(sensor->slv_addr, 0x32F5, regs[2]) || write_reg(sensor->slv_addr, 0x3060, regs[3]);

    if (ret == 0)
    {
        log_d("Set special_effect to: %d", effect);
...
static int set_brightness(sensor_t *sensor, int level)
{
    ...
    if (ret == 0)
    {
        log_d("Set brightness to: %d", level);
...
static int set_saturation(sensor_t *sensor, int level)
{
    ...
    if (ret == 0)
    {
        log_d("Set saturation to: %d", level);
...
static int set_sharpness(sensor_t *sensor, int level)
{
    ...
    if (ret == 0)
    {
        log_d("Set sharpness to: %d", level);
...
{
    log_d("set_gainceiling can not be configured at present");
    return 0;
}
...
{
    log_d("set_denoise can not be configured at present");
    return 0;
}
...
{
    int ret = 0;
    ret = write_addr_reg(sensor->slv_addr, X_ADDR_ST_H, startX, startY) || write_addr_reg(sensor->slv_addr, X_ADDR_END_H, endX, endY) || write_addr_reg(sensor->slv_addr, X_OFFSET_H, offsetX, offsetY) || write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, totalX, totalY) || write_addr_reg(sensor->slv_addr, X_OUTPUT_SIZE_H, outputX, outputY);
...
static int set_xclk(sensor_t *sensor, int timer, int xclk)
{
    ...
    if (xclk > 10)
    {
        log_e("only XCLK under 10MHz is supported, and XCLK is now set to 10M");
        xclk = 10;
    }
